...
def set_wallpaper():
    subprocess.Popen(["feh", "--bg-fill", "/home/charlie/Pictures/Wallpapers"])
...
keys = [
    ...
    Key([mod], "Escape", lazy.spawn("/home/charlie/.config/qtile/powermenu.sh"), desc="Show power menu"),
...
def get_wifi_status():
    try:
        # Get all WiFi networks and their connection status
        ...
        # Parse output to find the active connection
        for line in result.stdout.splitlines():
            if line.startswith("yes:"):
                return line.split(":")[1]  # Return the SSID
            
        return "OFF"  # No active connection found
...
def get_bt_status():
    try:
        ...
        if "Powered: yes" in result.stdout:
            # Check if actually connected to any device
            conn_result = subprocess.run(
                ["bluetoothctl", "info"],
                capture_output=True, text=True
            )
            if "Connected: yes" in conn_result.stdout:
                return "CONN"
            return "ON"
        return "OFF"
    except:
